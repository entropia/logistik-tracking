<script lang="ts">
    ...
    import type { UpdateCrateType } from "$lib/schemas/crates";
    ...
    let {
        ...
        data: SuperValidated<Infer<UpdateCrateType>>;
        ...
        <GenericFormField superform={sf} field="jiraTicket">
            {#snippet children({ labProps, inpProps })}
                ...
                <div class="flex flex-row gap-1">
                    ...
                    {#if $form.jiraTicket}
                        <Button size="icon" target="_blank" variant="outline" href={"https://jira.gulas.ch/browse/LOC-" + $form.jiraTicket}>
                            <img src="/jira.svg" class="h-[2em]" alt="Jira Logo" />
                        </Button>
                    {/if}
                </div>
            {/snippet}
